...
def interior(arr, dim, grid):
    if grid == "airfoil":
        ...
        wall_rows_per_row = int(width / w_width)
        wall_block_end = int(height / wall_rows_per_row)
        down_rows_per_row = int(width / d_width)
        ...
        b_1 = np.empty([0, w_width], float)
        for i in range(0, block1.shape[0]):
            line = block1[i, :]
            line = line.reshape([wall_rows_per_row, w_width])
            b_1 = np.append(b_1, line, axis=0)

        block2 = arr[end_block1:end_block2, :]
        b_2 = np.empty([0, w_width], float)
        ...
        b_3 = np.empty([0, d_width], float)
        for i in range(0, block3.shape[0]):
            line = block3[i, :]
            line = line.reshape([down_rows_per_row, d_width])
            b_3 = np.append(b_3, line, axis=0)

        block4 = arr[end_block3:end_block4, :]

        b_4 = np.empty([0, w_width], float)
...
def boundary(arr, dim, grid, interior, bType):
    if grid == "1b_o_grid":
        ...
        if bType == "bottom":
            ...
            b_1 = arr[0:w_width]
            b_2 = arr[w_width:2 * w_width]
            b_3 = arr[2 * w_width:3 * w_width]
            b_4 = arr[3 * w_width:4 * w_width]
            ...
            int1 = interior[0, -d_width:]
            int1 = np.flip(int1, axis=0)
            int2 = interior[0, :d_width]
        ...
        if bType == "top":
            ...
            b_1 = arr[0:w_width]
            b_2 = arr[w_width:2 * w_width]
            b_3 = arr[2 * w_width:2 * w_width + d_width]
            b_4 = arr[2 * w_width + d_width:3 * w_width + d_width]
            b_5 = arr[3 * w_width + d_width:4 * w_width + d_width]
            b_6 = arr[4 * w_width + d_width:4 * w_width + 2 * d_width]
